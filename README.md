...
An AI-native customer support platform (Zendesk / Intercom / Freshdesk class)
built as a TypeScript + Python monorepo.
...
│   └── semantic-cache/ Java 21 + Spring Boot semantic response cache in front of the RAG/LLM path
├── packages/
...
`apps/semantic-cache` is an optional optimization layer: it reuses a previously generated reply
when a new question is semantically equivalent *and* was produced under the same org, model,
prompt, knowledge-base version and retrieval config. The AI service calls it before running
retrieval, and falls back to the normal RAG pipeline whenever it is unavailable — nothing breaks
if the service is stopped. See [apps/semantic-cache/README.md](./apps/semantic-cache/README.md).
