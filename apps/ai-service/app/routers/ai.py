...
def delete_article(article_id: str, org_id: str | None = None) -> DeleteArticleResponse:
    # org_id is optional so existing callers keep working; when supplied it lets us invalidate
    # that org's cached answers, which may have been grounded in the article being removed.
    try:
        return remove_knowledge_article(article_id, org_id)
