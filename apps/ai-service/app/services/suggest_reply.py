...
from app.vector_store import embed_query, retrieve
...
# Existing retrieval relevance behavior.
# Do NOT change this when tuning source article display.
MIN_RELEVANCE_SCORE = 0.0

# Minimum similarity required for an article to be displayed as a UI source.
#
# IMPORTANT:
# This only controls source_articles returned to the frontend.
# It does NOT change:
# - Qdrant retrieval
# - RAG context
# - Gemini generation
# - SemantiCache
#
# Tune this value using real retrieval scores from your environment.
SOURCE_MIN_SIMILARITY = 0.65
...
def _extract_source_articles(
    ...
    """
    Extract only sufficiently relevant KB articles for display as
    source badges in the frontend.

    IMPORTANT:
    This function does NOT control RAG retrieval or the context sent
    to Gemini. It only controls which retrieved articles are exposed
    through `source_articles`.
    """
    ...
    for node in nodes:
        ...
        #
        # This prevents weak/irrelevant retrieved articles from
        # appearing underneath the AI suggested reply.
        #
        # IMPORTANT:
        # This does NOT remove the article from RAG context because
        # this function runs AFTER retrieval and generation.
        # ------------------------------------------------------------

        if similarity < SOURCE_MIN_SIMILARITY:
            logger.debug(
                "Source article filtered | article_id=%s | "
                "title=%s | similarity=%s | threshold=%s",
                article_id,
                metadata.get("title"),
                similarity,
...
# SEMANTIC CACHE
#
# The cache sits in front of retrieval and generation. A hit skips both.
# Everything below is best-effort: if any of it fails, the request carries
# on into the normal RAG pipeline.
# ============================================================================
...
JUDGE_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You compare two customer support questions. Answer YES only if the exact same "
            "reply would fully and correctly answer both. Answer NO if one is narrower, mentions "
            "a different product, plan, error code, account or time period, or would need any "
            "detail the other does not. Reply with one word: YES or NO.",
        ),
        (
            "human",
            "Question A:\n{question_a}\n\nQuestion B:\n{question_b}",
...
class _CacheState:
    ...
    response: SuggestReplyResponse | None = None


@lru_cache
def _prompt_version() -> str:
    """
    Hash of the rendered system prompt, so editing REPLY_PROMPT retires
    answers written under the old one without anyone having to remember
    to bump a constant.
    """

    rendered = "".join(
        str(message.content)
        for message in REPLY_PROMPT.format_messages(
            transcript="",
            context="",
        )
    )

    return semantic_cache.prompt_version(
        rendered
    )


@lru_cache
def _retrieval_hash() -> str:
    return semantic_cache.generation_config_hash(
        semantic_cache.retrieval_config_hash(),
        MIN_RELEVANCE_SCORE,
    )


@lru_cache
def _generation_hash() -> str:
    return semantic_cache.generation_config_hash(
        MAX_REPLY_WORDS
    )


def _judge_equivalent(
    query: str,
    cached_query: str,
) -> bool:
    """
    Second opinion on a borderline match.

    One cheap call instead of retrieval + generation.
    """
    ...
    try:
        ...
        logger.info(
            "Semantic judge verdict: %s",
            "reuse" if verdict else "regenerate",
        )
        ...
        logger.warning(
            "Semantic judge failed, regenerating instead",
            exc_info=True,
        )
...
def _cache_lookup(
    ...
    if not semantic_cache.is_enabled() or not query:
        return state

    try:
        state.embedding = embed_query(
            query
        )

    except Exception:
        # Without a vector there is no cache lookup,
        # and retrieval will embed the query itself.
        logger.warning(
            "Query embedding failed, skipping the semantic cache",
            exc_info=True,
        )
    ...
    result = semantic_cache.lookup(
        ...
        prompt_version_hash=_prompt_version(),
    ...
    state.lease_id = result.get(
        "leaseId"
    )

    if result.get("hit"):
        decoded = semantic_cache.decode_payload(
            result.get("response") or ""
        )

        if decoded:
            state.match_type = result.get(
                "matchType"
            )

            state.response = SuggestReplyResponse(
                **decoded
            )
    ...
    if (
        candidate
        and get_settings().SEMANTIC_JUDGE_ENABLED
    ):
        if _judge_equivalent(
            query,
...
def _cache_store(
    ...
    if (
        not semantic_cache.is_enabled()
        or state.embedding is None
    ):
        return

    # The fallback usually means a transient LLM failure or a gap in
    # the knowledge base. Caching it would keep serving "we could not
    # find this" for the whole TTL after the cause is fixed.
    if result.suggested_reply == FALLBACK_REPLY:
        semantic_cache.release(
            state.lease_id or "",
            "NOT_CACHEABLE",
        )
    ...
    semantic_cache.store(
        ...
        query=query,
        embedding=state.embedding,
        response_payload=semantic_cache.encode_payload(
...
def suggest_reply(
    ...
    # SemantiCache
    #
    # A hit skips retrieval and generation entirely. A miss may hand us a
    # lease, which makes us the owner of this query's execution while
    # identical concurrent requests wait on our answer.
    # ------------------------------------------------------------------------

    cache_state = _cache_lookup(
        request.org_id,
        query,
        language,
    )

    if cache_state.response is not None:
        logger.info(
            "suggest_reply served from cache | org_id=%s | match=%s",
            request.org_id,
            cache_state.match_type,
        )

        # A judged match is a new question pointing at an existing answer.
        # Storing it under this query's key turns the next occurrence into
        # an exact hit, and releases anyone waiting.
        if cache_state.match_type == "JUDGED":
            _cache_store(
                request.org_id,
                query,
                language,
        ...
        return cache_state.response

    # ------------------------------------------------------------------------
    # KB retrieval
    #
    # The embedding computed for the cache lookup is reused here, so a miss
    # does not pay for a second embedding call.
    # ------------------------------------------------------------------------

    retrieval = _run_retrieval(
        request.org_id,
        query,
        cache_state.embedding,
    )
    ...
    #
    # Only sufficiently relevant articles are returned to the frontend.
    # ------------------------------------------------------------------------
