...
def embed_query(query_text: str) -> list[float]:
    """Exposed so callers that need the query vector for something else (the semantic cache)
    can pass it back into retrieve() instead of paying for a second embedding call."""
    return get_embedding_model().embed_query(query_text)


def retrieve(org_id: str, query_text: str, top_k: int | None = None, embedding: list[float] | None = None):
    """Returns LlamaIndex NodeWithScore results, scoped to one organization.

    When an embedding is supplied it is used as-is, so a caller that already embedded the query
    does not make the provider embed it twice."""
    settings = get_settings()
    index = get_index()
    ...
    return retriever.retrieve(QueryBundle(query_str=query_text, embedding=embedding))
