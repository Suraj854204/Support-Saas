...
def semantic_cache_off(monkeypatch):
    """The existing suite exercises the RAG path, so the semantic cache is opt-in per test.
    This also keeps the suite from reaching out to a service that is not running."""
