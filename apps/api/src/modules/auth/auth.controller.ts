...
export const authController = {
  async register(req: Request<unknown, unknown, RegisterInput>, res: Response) {
    ...
    const body: ApiResponse<{ user: ReturnType<typeof toPublicUser>; org: typeof org; accessToken: string }> = {
      success: true,
      data: { user: toPublicUser(user), org, accessToken: tokens.accessToken },
    };
  ...
  async login(req: Request<unknown, unknown, LoginInput>, res: Response) {
    ...
    setRefreshCookie(res, tokens.refreshToken);
    const body: ApiResponse<{ user: ReturnType<typeof toPublicUser>; accessToken: string }> = {
      success: true,
