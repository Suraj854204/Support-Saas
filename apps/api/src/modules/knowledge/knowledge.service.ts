...
async function syncToIndex(orgId: string, articleId: string) {
  ...
  if (article.status !== "published") {
    ...
    if (article.vectorIndexed) {
      await aiClient.bestEffort(() => aiClient.deleteArticle(articleId, orgId), "knowledge.unpublish");
...
export const knowledgeService = {
  ...
  async delete(orgId: string, articleId: string) {
    await this.getById(orgId, articleId);
    await aiClient.bestEffort(() => aiClient.deleteArticle(articleId, orgId), "knowledge.delete");
