...
export const teamsService = {
  async list(orgId: string) {
    return prisma.team.findMany({
      where: { orgId },
      include: { members: { include: { user: true } }, manager: true },
  ...
  async setManager(orgId: string, teamId: string, managerId: string | null) {
    await this.getById(orgId, teamId);

    if (managerId) {
      const manager = await prisma.user.findFirst({ where: { id: managerId, orgId } });
      if (!manager) throw AppError.notFound("User not found in this organization");
    }

    return prisma.team.update({ where: { id: teamId }, data: { managerId } });
  },
