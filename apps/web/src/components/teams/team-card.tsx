...
export function TeamCard({ team }: { team: TeamWithMembers }) {
  ...
  const setManager = useSetTeamManager();
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {isAdmin ? (
            <Select
              ...
              onValueChange={(v) =>
                setManager.mutate(
                  { teamId: team.id, managerId: v === NO_MANAGER ? null : v },
                  { onError: () => toast.error("Couldn't set manager.") }
                )
              }
            >
            ...
            <span className="text-xs">{team.manager?.name ?? "No manager"}</span>
          )}
        </div>
